  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className={`${cls} ${i < rating ? "text-gold" : "text-dark-border"}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function ReviewsPage() {
  ...
  // Fetch from GET /api/v1/testimonials
  useEffect(() => {
    let cancelled = false;
    ...
    api.fetchTestimonials()
      .then((data) => {
        if (cancelled) return;
        const mapped: Review[] = data.map((t) => ({
          id: t.id,
          guest: t.guestName,
          avatar: t.guestInitials ?? t.guestName.split(" ").map((n) => n[0]).join(""),
          hotel: t.hotelName,
          room: t.roomType ?? "—",
          rating: t.rating,
          title: t.title ?? "",
          comment: t.comment,
          date: t.date,
          status: t.status === "published" ? "Published" : t.status === "pending" ? "Pending" : "Flagged",
  ...
  const filtered = reviews.filter((r) => {
    ...
    const matchesRating =
      ...
      (ratingFilter === "3 Stars & Below" && r.rating <= 3);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-dark-card border border-dark-border rounded-xl p-5">
          <div className="flex items-center gap-4">
            <div className="text-center">
              ...
              <StarRating rating={5} />
              <p className="text-dark-muted text-xs mt-1">{reviews.length} reviews</p>
            </div>
        ...
        <div className="grid grid-cols-3 gap-4 lg:col-span-2">
          {[
            { label: "Total Reviews", value: reviews.length.toString(), icon: "⭐" },
            { label: "Published", value: reviews.filter((r) => r.status === "Published").length.toString(), icon: "✅" },
            { label: "Pending", value: reviews.filter((r) => r.status === "Pending").length.toString(), icon: "⏳" },
            { label: "Flagged", value: reviews.filter((r) => r.status === "Flagged").length.toString(), icon: "🚩" },
            { label: "Avg Rating", value: reviews.length ? (reviews.reduce((a, r) => a + r.rating, 0) / reviews.length).toFixed(1) : "—", icon: "📊" },
            { label: "Source", value: apiStatus === "live" ? "API" : "Local", icon: apiStatus === "live" ? "🔗" : "💾" },
          ].map((s) => (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 items-center">
          ...
          {apiStatus === "live" && <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded-full">Live · /testimonials</span>}
          <select value={ratingFilter} onChange={(e) => setRatingFilter(e.target.value)}
            className="bg-dark-card border border-dark-border text-dark-muted text-xs rounded-lg px-3 py-1.5 focus:outline-none focus:border-gold">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div>
                      ...
                      <p className="text-dark-muted text-xs mt-0.5">{r.hotel} • {r.room} • {r.date}</p>
                    </div>
                    <div className="flex gap-1.5 shrink-0">
                      {r.status === "Pending" && (
                        <button onClick={() => approve(r.id)} className="flex items-center gap-1 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-medium rounded-lg hover:bg-emerald-500/20 transition-colors">
                      ...
                      {r.status === "Flagged" && (
                        <button onClick={() => remove(r.id)} className="flex items-center gap-1 px-2.5 py-1 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-medium rounded-lg hover:bg-red-500/20 transition-colors">
                          Remove
                        </button>
                      )}
                      <button onClick={() => remove(r.id)} className="p-1.5 rounded-md bg-dark border border-dark-border text-dark-muted hover:text-red-400 transition-colors">
